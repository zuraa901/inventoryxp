...
<body>
        ...
        <div class="main" id="main">
            <div class="topbar">
                <div class="topbar-left">
                    ...
                    <div class="topbar-title">Asset</div>
                </div>
            ...
            <div class="content-card">
                ...
                <button type="button" onclick="openModal()" class="add-btn">
                    + Tambah Asset
                </button>
                       ...
                       @forelse ($assets as $index => $asset)
                            <tr>
                                ...
                                <td class="action-cell">
                                    <button type="button" class="row-menu-trigger" onclick="toggleMenu(event, 'menu-{{ $asset->id }}')">⋮</button>

                                        <div class="row-menu" id="menu-{{ $asset->id }}">
                                            <button type="button"
                                        ...
                                        <form action="/asset/delete/{{ $asset->id }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this asset?');">
                                            @csrf
                        ...
                        @empty
                            <tr>
                                <td colspan="6">No asset data found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div id="modalOverlay" style="position:fixed; inset:0; background:rgba(0,0,0,0.45); display:{{ $errors->any() ? 'flex' : 'none' }}; justify-content:center; align-items:center; padding:20px; z-index:999;">
        <div style="background:#fff; width:100%; max-width:760px; border-radius:16px; overflow:hidden; box-shadow:0 8px 24px rgba(0,0,0,0.18);">
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; padding:22px 28px; border-bottom:1px solid #ddd;">
                <h3 style="font-size:22px; font-weight:normal;">Tambah Asset</h3>
            ...
            <form action="/asset/store" method="POST">
                ...
                <div style="padding:26px 28px 18px;">
                    ...
                    <div style="margin-bottom:22px;">
                        <label for="kode_asset" style="display:block; margin-bottom:10px;">Kode Asset</label>
                        <input type="text" id="kode_asset" name="kode_asset" value="{{ old('kode_asset') }}" style="width:100%; padding:14px 16px; border:1px solid #d5dbe1; border-radius:10px;">
                    </div>

                    <div style="margin-bottom:22px;">
                        <label for="nama_asset" style="display:block; margin-bottom:10px;">Nama Asset</label>
                        <input type="text" id="nama_asset" name="nama_asset" value="{{ old('nama_asset') }}" style="width:100%; padding:14px 16px; border:1px solid #d5dbe1; border-radius:10px;">
                    </div>

                    <div style="margin-bottom:22px;">
                        <label for="status" style="display:block; margin-bottom:10px;">Status</label>
                        <input type="text" id="status" name="status" value="{{ old('status') }}" style="width:100%; padding:14px 16px; border:1px solid #d5dbe1; border-radius:10px;">
                    </div>

                    <div style="margin-bottom:22px;">
                        <label for="lokasi" style="display:block; margin-bottom:10px;">Lokasi</label>
                        <input type="text" id="lokasi" name="lokasi" value="{{ old('lokasi') }}" style="width:100%; padding:14px 16px; border:1px solid #d5dbe1; border-radius:10px;">
                    </div>
                </div>
    ...
    <script>
        ...
        function openEditModal(id, kode, nama, status, lokasi) {
            document.getElementById('editForm').action = '/asset/update/' + id;
            document.getElementById('edit_kode_asset').value = kode;
            document.getElementById('edit_nama_asset').value = nama;
   ...
   <div class="modal-overlay" id="editModal">
        <div class="modal-box">
            ...
            <form id="editForm" method="POST" onsubmit="return confirm('Are you sure you want to update this asset?');">
                @csrf

                <div class="modal-body">
                    <div class="form-group">
                        <label for="edit_kode_asset">Kode Asset</label>
                        <input type="text" id="edit_kode_asset" name="kode_asset">
                    </div>

                    <div class="form-group">
                        <label for="edit_nama_asset">Nama Asset</label>
                        <input type="text" id="edit_nama_asset" name="nama_asset">
                    </div>
